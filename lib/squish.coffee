...
reflectNewUrl = (url) ->
  # if url isnt document.location.toString)()
  referer = document.location.href
  if url.match(/\.html$/)
    url = url.slice(0, -5)
  if url.match(/index$/)
...
reflectRedirectedUrl = (newUrl) ->
  newUrl = "/" + newUrl
  if newUrl isnt document.location.pathname + document.location.search
    if newUrl.match(/\.html$/)
      newUrl = newUrl.slice(0, -5)
    if newUrl == "index"
      newUrl = "/"
    window.history.replaceState currentState, '', newUrl + document.location.hash
